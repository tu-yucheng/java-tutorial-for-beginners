...
有时，我们需要根据所运行的操作系统(OS)更改我们的测试场景。在这些情况下，@EnabledOnOs注解能派上用场。
...
此外，如果我们想禁用使用8、11、17以外的Java版本运行的测试，我们可以使用JRE枚举的OTHER属性：
...
基于脚本的条件API及其实现在JUnit 5.5中已弃用，并从JUnit 5.6中删除。
为了达到相同的效果，建议使用内置条件的组合或创建ExecutionCondition的自定义实现。
...
+ value - 包含要运行的实际脚本。
+ engine(可选) - 指定要使用的脚本引擎，默认为Oracle Nashorn。
+ reason(可选) - 用于日志记录，指定测试失败时JUnit应打印的消息。

让我们来看一个简单的例子，其中我们只指定一行脚本，注解中没有配置其他参数：
...
+ junitDisplayName - 测试的显示名称。
+ junitTags - 访问测试或容器上的Tags。
